...
namespace SAMS.BLL.Pipeline
{
    //instead of repeating so much code so that we get specific objects from the service layer we have created a single pipeline 
    //this pipeline will be used to get data as a string then we convert these strings to objects by the specified method
    public class Pipeline
    {
        ...
        public static string Patch(string session, string data, string Object)
        {
            ...
            {
                string sessionId = SessionBLL.ValidateAppSession(session);
                string ddfJson = SLY.Class1.Patch2(Globals.Globals.AuthSessions[sessionId].B1Cookie, Object, data);
